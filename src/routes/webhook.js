  ...
  try {
    ...
    const reforcoAutoReconhecimento = referenciaVisao
      ? contextoSugereElaPropriaFoto
        // Contexto textual forte — assume que é ela
        ...
        // Sem contexto — comparação visual: permissiva quando há rosto parecido, conservadora pra não-pessoas
        : `\n\nVOCÊ TEM UMA FOTO DE REFERÊNCIA SUA JUNTO NESTA MENSAGEM (a primeira imagem é você, Clara — a segunda é a foto que o usuário mandou). ANTES de comparar: se a segunda imagem NÃO mostrar uma pessoa real (for cartaz, arte, texto, objeto, paisagem) — IGNORE a referência e analise normalmente, pois não pode ser você. Se mostrar uma mulher jovem com traços parecidos com os seus (cabelo escuro, pele morena, jovem, traços latinos) — provavelmente é você, reaja como se reconhecendo na foto. Não precisa ter certeza absoluta — se parece você, é provável que seja. Só descarte se for claramente outra pessoa (homem, criança, idosa, loira, etc) ou sem pessoa alguma.`
      : '';
