...
.drawer-nav{padding:4px 8px;flex:1;overflow-y:auto}
...
/* FIX 2: input-area usa padding-bottom dinâmico para não sumir atrás do teclado no iOS PWA */
.input-area{
  padding:12px 20px 16px;
  padding-bottom:max(16px, env(keyboard-insets-bottom, 16px));
  background:var(--bg);
...
/* FIX 1: nome do remédio não corta mais — quebra linha normalmente */
.med-row{display:flex;align-items:flex-start;gap:12px;padding:14px 16px;background:var(--s2);border:1px solid var(--border);border-radius:var(--r);margin:0 20px 10px;transition:all .15s}
.med-row.concluido{opacity:.4;filter:saturate(.3)}
.med-icon-wrap{width:40px;height:40px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px}
...
.med-name{font-size:13px;font-weight:600;color:var(--text);white-space:normal;word-break:break-word;line-height:1.4}
...
.med-action-btn{padding:7px 13px;border:none;border-radius:var(--r-sm);font-family:'Inter',sans-serif;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;flex-shrink:0;align-self:center}
...
<div class="drawer" id="drawer">
  <div class="drawer-head"><div class="drawer-user-avatar" id="drawer-avatar">✨</div><div class="drawer-user-info"><div class="drawer-user-name" id="drawer-name">Usuário</div><div class="drawer-user-phone" id="drawer-phone">—</div></div><button class="drawer-close" onclick="closeDrawer()">✕</button></div>
  ...
  <div class="drawer-foot">
    <button class="d-foot-btn" id="notif-btn" onclick="requestPushPermission()"><svg viewBox="0 0 24 24"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg> Notificações</button>
...
function renderRem(){const el=document.getElementById('rem-timeline');const now=new Date(),todayStr=dateBRTStr(now),viewStr=dateBRTStr(remViewDate),isToday=viewStr===todayStr;const dias=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'],meses=['janeiro','fevereiro','março','abril','maio','junho','julho','agosto','setembro','outubro','novembro','dezembro'];const offset=-3*60,utc=remViewDate.getTime()+remViewDate.getTimezoneOffset()*60000,vd=new Date(utc+offset*60000);document.getElementById('rem-date-label').textContent=isToday?'Hoje':`${dias[vd.getDay()]}, ${pad(vd.getDate())} de ${meses[vd.getMonth()]}`;document.getElementById('rem-date-sub').textContent=isToday?`${dias[vd.getDay()]}, ${pad(vd.getDate())} de ${meses[vd.getMonth()]}` :'';document.getElementById('rem-today-btn').style.display=isToday?'none':'';const dayRems=allRem.filter(r=>dateBRTStr(r.scheduledAt)===viewStr);const pending=dayRems.filter(r=>!r.confirmed&&!r.sent).sort((a,b)=>new Date(a.scheduledAt)-new Date(b.scheduledAt));const done=dayRems.filter(r=>r.confirmed||r.sent).sort((a,b)=>new Date(a.scheduledAt)-new Date(b.scheduledAt));if(!dayRems.length){el.innerHTML='<div class="empty"><span class="empty-icon">📅</span>Nenhum lembrete neste dia</div>';return;}let html='<div style="padding-top:8px">';let nowInserted=false;pending.forEach(r=>{const t=new Date(r.scheduledAt);const hm=`${pad(t.getHours())}:${pad(t.getMinutes())}`;if(isToday&&!nowInserted&&t>now){nowInserted=true;html+=`<div class="rem-now-line"><span class="rem-now-time">${pad(now.getHours())}:${pad(now.getMinutes())}</span><div class="rem-now-bar"></div><div class="rem-now-dot"></div></div>`;}html+=`<div class="rem-row" id="ri-${r.id}"><span class="rem-row-time">${hm}</span><div class="rem-row-check" onclick="doneRem('${r.id}')"></div><div class="rem-row-title">${r.message}</div><button class="rem-del-btn" onclick="delRem('${r.id}')" title="Excluir">✕</button></div>`;});if(isToday&&!nowInserted)html+=`<div class="rem-now-line"><span class="rem-now-time">${pad(now.getHours())}:${pad(now.getMinutes())}</span><div class="rem-now-bar"></div><div class="rem-now-dot"></div></div>`;if(done.length)html+=`<div style="margin-top:4px"><button class="rem-done-toggle" id="done-toggle" onclick="toggleDoneSection()"><svg viewBox="0 0 14 14"><path d="M3 5l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" fill="none"/></svg> Concluídos (${done.length})</button><div id="done-items" style="display:none">${done.map(r=>{const t=new Date(r.scheduledAt);return`<div class="rem-row done-row"><span class="rem-row-time" style="color:var(--text3)">${pad(t.getHours())}:${pad(t.getMinutes())}</span><div class="rem-row-check done"></div><div class="rem-row-title done">${r.message}</div></div>`;}).join('')}</div></div>`;html+='</div>';el.innerHTML=html;}
...
function renderSaude(){const el=document.getElementById('saude-body');if(!allMeds.length){el.innerHTML='<div class="empty"><span class="empty-icon">💊</span>Nenhum medicamento cadastrado</div>';return;}const now=new Date(),hm=`${pad(now.getHours())}:${pad(now.getMinutes())}`;const pendentes=[],concluidos=[];allMeds.forEach((m,i)=>{let times=[];try{times=JSON.parse(m.times);}catch{}const nextToday=times.find(t=>t>=hm);if(nextToday){pendentes.push({m,i,nextTime:nextToday,nextDay:'Hoje'});}else{concluidos.push({m,i,nextTime:times[0]||'—',nextDay:times.length?'Amanhã':'—'});}});let html='';if(pendentes.length){html+=`<div class="med-section-label">Próximas doses</div>`;pendentes.forEach(({m,i,nextTime,nextDay})=>{const ci=i%MED_COLORS.length;html+=`<div class="med-row"><div class="med-icon-wrap" style="background:${MED_COLORS[ci]};color:${MED_TEXT[ci]}"><svg viewBox="0 0 24 24"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg></div><div class="med-info"><div class="med-name">${m.name}</div><div class="med-freq">${m.frequency}x por dia</div></div><div class="med-next"><div class="med-next-label">Próxima dose</div><div class="med-next-time" style="color:${MED_TEXT[ci]}">${nextTime}</div><div class="med-next-day">${nextDay}</div></div><div class="med-doses"><div class="med-doses-num">${m.remaining}</div><div>doses rest.</div></div><button class="med-action-btn tomar" onclick="takeMed('${m.id}',this)">Tomar agora</button><button class="med-del-btn" onclick="delMed('${m.id}')" title="Excluir">✕</button></div>`;});}if(concluidos.length){html+=`<div class="med-section-label" style="margin-top:8px;color:var(--green);opacity:.7">✓ Concluídos hoje</div>`;concluidos.forEach(({m,i,nextTime,nextDay})=>{const ci=i%MED_COLORS.length;html+=`<div class="med-row concluido"><div class="med-icon-wrap" style="background:${MED_COLORS[ci]};color:${MED_TEXT[ci]}"><svg viewBox="0 0 24 24"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg></div><div class="med-info"><div class="med-name">${m.name}</div><div class="med-freq">${m.frequency}x por dia</div></div><div class="med-next"><div class="med-next-label">Próxima dose</div><div class="med-next-time" style="color:${MED_TEXT[ci]}">${nextTime}</div><div class="med-next-day">${nextDay}</div></div><div class="med-doses"><div class="med-doses-num">${m.remaining}</div><div>doses rest.</div></div><button class="med-action-btn concluido-btn" disabled>✓ Tomado</button></div>`;});}if(!pendentes.length&&!concluidos.length)html='<div class="empty"><span class="empty-icon">💊</span>Nenhum medicamento</div>';html+=`<div class="med-tip"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><div><div class="med-tip-title">Dica da Clara</div><div class="med-tip-text">Mantenha sua rotina de medicamentos em dia. Isso faz toda a diferença!</div></div></div>`;el.innerHTML=html;}
