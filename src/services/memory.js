...
async function savePurchase(userId, item) {
  const existing = await prisma.purchase.findFirst({
    where: { userId, item: { contains: item, mode: 'insensitive' } },
  });
  if (existing) {
    const daysSinceLast = Math.floor(
      (Date.now() - existing.lastBought.getTime()) / (1000 * 60 * 60 * 24)
    );
    const newAvg = existing.avgFrequency
      ? Math.round((existing.avgFrequency + daysSinceLast) / 2)
      : daysSinceLast;
...
async function saveTask(userId, data) {
  ...
  const task = await prisma.task.create({
    data: {
      ...
      title: titulo,
      dueDate,
      dueTime: hora && hora !== 'null' ? hora : null,
      items: itens && itens !== 'null' ? itens : null,
    },
  });
...
async function saveSleepLog(userId, data) {
  ...
  const log = await prisma.sleepLog.create({
    data: {
      userId,
      bedtime: horario_dormir || null,
      wakeTime: horario_acordar || null,
      quality: qualidade || null,
      hours: horas || null,
    },
  });
...
async function saveWorkout(userId, data) {
  ...
  const workout = await prisma.workout.create({
    data: {
      ...
      exercises: exercicios || null,
      note: nota || null,
    },
  });
  await saveMemory(userId, 'treino', `${modalidade}${duracao ? ` - ${duracao}min` : ''}`, {
...
async function saveEvent(userId, data) {
  ...
  const event = await prisma.event.create({
    data: {
      userId,
      title: titulo,
      personName: pessoa || null,
      date: eventDate || new Date(),
      type: type || 'outro',
    },
  });
  await saveMemory(userId, 'evento', `${titulo}${pessoa ? ` - ${pessoa}` : ''}`, {
    eventId: event.id,
  });
  return event;
}
