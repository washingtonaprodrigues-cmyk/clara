// Clara memory v4.0 - Versão limpa para Railway
...
async function getJornada(userId) {
  ...
  return user?.jornadaMinutos || 480; // 8 horas padrão
}
...
async function savePurchase(userId, item) {
  ...
  if (existing) {
    ...
    return { purchase: updated, isRecurring: true };
  }
  const purchase = await prisma.purchase.create({ data: { userId, item } });
  await saveMemory(userId, 'compra', item);
  return { purchase, isRecurring: false };
}
...
// Adicione aqui outras funções que você usar no futuro (saveSecret, saveNote, etc.)
