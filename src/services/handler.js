...
async function handleMessage(phone, text, location = null) {
  try {
    ...
    if (textLower === 'ver lembretes') return await listarLembretes(user, phone);
    if (textLower === 'ver anotações' || textLower === 'ver anotacoes') return await listarAnotacoes(user, phone);
    if (textLower === 'ver gastos') return await listarGastos(user, phone);
    if (textLower === 'ver horas hoje' || textLower === 'bater ponto') return await listarPontoHoje(user, phone);
    if (textLower === 'ver medicamentos') return await listarMedicamentos(user, phone);
...
async function handleTask(user, phone, classified) {
  ...
  if (classified.hora) {
    try {
      const hoje = classified.data || new Date().toISOString().split('T')[0];
...
async function handleExpense(user, phone, classified) {
  ...
  const icon = categoriaIcon[classified.categoria] || '📦';

  await sendButtons(phone,
    `💰 *Gasto registrado!*\n\n${icon} *${classified.categoria.charAt(0).toUpperCase() + classified.categoria.slice(1)}*\n💵 R$ ${classified.valor.toFixed(2)}\n\nSeu gasto foi salvo no controle financeiro 😊`,
    [
