...
async function handleSaudacao(user, phone) {
 console.log(`[MENU] Enviando menu para ${phone}`);
 try {
   const cidade = await getCidadeUsuario(user.id);
   const result = await sendMainMenu(phone);
   console.log(`[MENU] Resultado Z-API:`, JSON.stringify(result));
   if (!cidade) {
     await sendMessage(phone, '📍 Dica: me diz sua cidade para eu buscar clima e locais pra você!');
   }
 } catch (err) {
   console.error(`[MENU] Erro:`, err.response?.data || err.message);
   await sendMessage(phone, 'Oi! 😊 Como posso te ajudar?');
 }
}
